  ...
  function getLastEmoji(value: string) {
    ...
    if (emojiMatches && emojiMatches.length > 0) {
      return emojiMatches[emojiMatches.length - 1];
    }

    return Array.from(trimmed).at(-1) ?? "";
  }

  function getSelectedEmoji(value: string) {
  const emoji = getLastEmoji(value);
  return emoji || "✨";
}
  ...
  export default function CreateStatus() {
    ...
    useEffect(() => {
      ...
      getPost(editId).then((post) => {
        ...
        if (isCustomActivity(post.status)) {
          ...
          setCustomEmoji(meta.emoji === "✨" ? "" : meta.emoji);
    ...
    const useCustomActivity = () => {
      ...
      const cleanEmoji = getSelectedEmoji(customEmoji);
          ...
          <section>
            ...
            >
              ...
              <div className="grid grid-cols-[64px_minmax(0,1fr)] gap-2">
                <Input
                  value={customEmoji}
                  onChange={(e) => setCustomEmoji(getLastEmoji(e.target.value))}
                  placeholder="✨"
                  maxLength={8}
              ...
              {customIsSelected && (
                <p className="mt-2 text-center text-xs text-muted-foreground">
                  Selected: {selectedActivityMeta.emoji}{" "}
                  {selectedActivityMeta.label}
                </p>
              )}
            </div>
          </section>
          ...
          <section className="space-y-2">
            ...
            <div className="w-full max-w-full overflow-hidden rounded-3xl border border-border bg-card p-4 shadow-sm">
              ...
              <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
                End earlier than start means it continues into the next day.
              </p>
            </div>
          </section>
