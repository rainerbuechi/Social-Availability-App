...
function getPinVariant(pin: MapPin, favoriteIds: string[]): { color: string; variant: MarkerVariant } {
  const isFav = favoriteIds.includes(pin.placeId ?? pin.id);
  if (isFav) return { color: "#DA2C43", variant: "favorite" };
  if (pin.source === "manual") return { color: "#DA2C43", variant: "community" };
...
function BoundsTracker({ onBoundsChange }: {
  ...
  useMapEvents({
    moveend: () => {
      ...
      onBoundsChange([b.getSouth(), b.getWest(), b.getNorth(), b.getEast()], { lat: c.lat, lng: c.lng });
    },
  });
  return null;
}
...
const ADD_CATEGORIES: PlaceCategory[] = [
  "cafe", "bar", "restaurant", "park", "sports", "viewpoint",
  "library", "study_spot", "event_venue", "public_space", "other",
];
...
export default function DiscoverMap({
  ...
  // FIX #4: ref to scroll the add-place form into view after dropping a pin
  ...
  // FIX #4: when pin is dropped, scroll up to the form automatically
  useEffect(() => {
    if (pendingLatLng && addFormRef.current) {
      setTimeout(() => {
  ...
  const CATEGORY_ORDER: PlaceCategory[] = [
    ...
    "study_spot", "public_space", "viewpoint", "museum", "event_venue", "other",
  ];

  const availableCategories = CATEGORY_ORDER.filter((c) => pins.some((p) => p.placeCategory === c));
  ...
  let visiblePins = activeCategories.size === 0
    ? pins
    : pins.filter((p) => p.placeCategory && activeCategories.has(p.placeCategory));

  if (myPlacesOnly) visiblePins = visiblePins.filter(
    (p) => p.source === "manual" || favoriteIds.includes(p.placeId ?? p.id),
  );
  if (communityOnly) visiblePins = visiblePins.filter((p) => p.source === "manual");

  const myCount = pins.filter((p) => p.source === "manual" || favoriteIds.includes(p.placeId ?? p.id)).length;
  ...
  return (
    <div className="space-y-2">
      {/* FIX #3: Refresh + Add place always get their own uncluttered row */}
      ...
      {/* FIX #3: Mine + Community live in the scrollable row with categories — no more cramping */}
      <div className="scrollbar-none flex gap-1.5 overflow-x-auto pb-0.5">
        {myCount > 0 && (
          <button
            onClick={() => setMyPlacesOnly((v) => !v)}
            className={`shrink-0 rounded-full border px-3 py-1 text-[11px] font-semibold shadow-sm transition-colors ${
              myPlacesOnly
      ...
      {/* FIX #4: ref attached here so we can scroll to it */}
      {pendingLatLng && (
        <div ref={addFormRef} className="space-y-3 rounded-3xl border border-border bg-card p-4 shadow-sm">
          ...
          <div className="flex items-center justify-between rounded-2xl border border-border bg-background px-3 py-2.5">
            ...
            <button
              ...
              className={`relative ml-3 inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors ${addIsPublic ? "bg-[#DA2C43]" : "bg-muted"}`}
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${addIsPublic ? "translate-x-6" : "translate-x-1"}`} />
            </button>
          </div>
      ...
      {suggestPin && (
        <div className="space-y-3 rounded-3xl border border-border bg-card p-4 shadow-sm">
          <p className="text-sm font-semibold">
            Suggest <span className="text-[#DA2C43]">"{suggestPin.title}"</span> to:
          </p>
      ...
      <div className="relative w-full" style={{ height: "calc(100dvh - 260px)", minHeight: 300 }}>
        <MapContainer
        ...
        >
          ...
          {pendingLatLng && (
            <Marker position={[pendingLatLng.lat, pendingLatLng.lng]} icon={makeIcon("#49769F", "pending")} />
          )}
          ...
          {visiblePins.map((pin) => {
            ...
            return (
              <Marker key={pin.id} position={[pin.lat, pin.lng]} icon={makeIcon(color, variant)}>
                <Popup minWidth={240} maxWidth={300}>
                  <div className="space-y-2.5 py-1 font-sans">
                    <div className="flex items-start gap-2">
                      <span className="mt-0.5 text-xl leading-none">{getPinEmoji(pin)}</span>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold leading-snug text-gray-900">{pin.title}</p>
                        <div className="mt-1 flex flex-wrap items-center gap-1.5">
                          ...
                          {isFav && <span className="text-[10px] font-semibold text-[#DA2C43]">❤️ Saved</span>}
                          {isOwn && <span className="text-[10px] font-semibold text-[#DA2C43]">👥 Community</span>}
                        </div>
                      </div>
                    </div>
                    ...
                    {pin.description
                      ? <p className="text-xs leading-snug text-gray-600">{pin.description}</p>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <button
                        onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(pin.address ?? pin.title)}`, "_blank")}
        ...
        <div className="pointer-events-none absolute bottom-8 left-2 z-[1000] flex items-center gap-2 rounded-full border border-border/50 bg-background/80 px-3 py-1 text-[10px] text-muted-foreground backdrop-blur-sm">
          <span className="flex items-center gap-1">
            <span className="inline-block h-2 w-2 rounded-full bg-[#DA2C43]" /> Community
          </span>
          <span className="flex items-center gap-1">
            <span className="inline-block h-2 w-2 rounded-full bg-[#DA2C43]" /> Saved
          </span>
          <span className="flex items-center gap-1">
            <span className="inline-block h-2 w-2 rounded-full bg-amber-500" /> External
          </span>
        </div>
      </div>
    </div>
